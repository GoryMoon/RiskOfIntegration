...
using R2API.Utils;
using RiskOfIntegration.Actions;
...
namespace RiskOfIntegration
{
    public class ActionManager
    {
        ...
        public ActionManager(ManualLogSource logger)
        {
            ...
            AddAction(typeof(TimeScale));
            AddAction(typeof(ChangeRuntime));
            AddAction(typeof(ChangeMoney));
            AddAction(typeof(KillAll));
        ...
        public void HandleAction(string rawAction)
        {
            ...
            try
            {
                ...
                {
                    action.TryAfter = DateTime.Now + TimeSpan.FromMilliseconds(UnityEngine.Random.Range(action.DelayMin, action.DelayMax));
                }
        ...
        public void Update()
        {
            ...
            switch (response)
            {
                ...
                case ActionResponse.Done:
                {
                    ...
                    {
                        ChatMessage.SendColored($"Ran action {action.GetType().Name.Humanize()}", "#d63031", action.From);
                    }*/
                    break;
                }
            }
